...
router.post(
  ...
  requireFeature("gdrive_sync"),
  mediumWriteLimiter,
  throttle(300, 5, "drive-disconnect"),
  disconnectGoogleDrive,
);
...
router.delete(
  ...
  requireFeature("gdrive_sync"),
  mediumWriteLimiter,
  throttle(300, 8, "drive-file-delete"),
  validate(deleteFromDriveSchema),
  deleteFromDrive,
);
// Upload a file into a Drive folder (or "root" for Drive root)
router.post(
  "/file/:parentFolderId/upload",
  checkAuth,
  loadPlanContext,
  requireFeature("gdrive_sync"),
  mediumWriteLimiter,
...
router.post(
  ...
  requireFeature("gdrive_sync"),
  mediumWriteLimiter,
  throttle(300, 8, "drive-move"),
  validate(moveDriveItemsSchema),
  moveDriveItems,
);
...
router.get(
  "/folder/:folderId/download",
  checkAuth,
  loadPlanContext,
  requireFeature("gdrive_sync"),
  heavyOpLimiter,
  throttle(5000, 1, "drive-folder-download"),
  validate(listDriveFolderSchema),
  downloadDriveFolder,
);
...
router.get(
  ...
  requireFeature("gdrive_sync"),
  searchLimiter,
  throttle(200, 10, "drive-search"),
  validate(searchDriveFilesSchema),
  searchDriveFiles,
);
