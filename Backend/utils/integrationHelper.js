...
export async function resolveIntegrationOwnerId(req) {
  ...
  // Get ownerId from query params, body, or headers safely
  const ownerId =
    req.query?.ownerId || req.body?.ownerId || req.headers?.["x-owner-id"];
...
export async function hasWriteAccess(ownerId, req) {
  ...
  if (
    sharedAccess &&
    (sharedAccess.permission.includes("write") ||
      sharedAccess.permission.includes("owner"))
  ) {
    return true;
  }
...
export async function verifyItemAccess(
  ...
  if (permissionNeeded === "write") {
    ...
    if (
      !sharedAccess.permission.includes("read") &&
      !sharedAccess.permission.includes("write") &&
      !sharedAccess.permission.includes("owner")
    ) {
      return false;
